> **SUPERSEDED FOR THE `pinn-advection-diffusion` ENTRY (2026-09-21).** The rows below record that the PINN figure and site content were unchanged in the earlier pass, and the previously displayed "~5×10⁻³ relative L² error" headline described the historical value-only formulation. They are retained as a historical record and are **not** the current claims.
>
> **Current state:** the `projects.html#pinn` entry now describes value-and-derivative periodic boundary enforcement (research repository `pinn-advection-diffusion` `c568e9fd3e1c3b2ef390b4b43a150fa14887ea76`). Its headline is a 7-38x reduction in boundary derivative mismatch across five seeds versus the value-only loss, with no reliable global L2 improvement at the original 15,000-epoch budget and a higher PDE residual for the corrected formulation. The existing figure is retained and captioned as the historical value-only result (seed 42).
